...
if(!function_exists('mbw_update_option')){
	function mbw_update_option($key,$value){
		...
		$mdb->query($mdb->prepare("update ".$mb_admin_tables["options"]." set ".$mb_fields["options"]["fn_option_value"]."=%s where `".$mb_fields["options"]["fn_option_name"]."`=%s;",$value,$key));
...
if(!function_exists('mbw_options_meta_refresh')){
	function mbw_options_meta_refresh(){
		if(mbw_is_admin()){
			...
			$option_data			= $mdb->get_results($mdb->prepare("select %1s,%1s from %1s;",$mb_fields["options"]["fn_option_name"],$mb_fields["options"]["fn_option_value"],$mb_admin_tables["options"]), ARRAY_A);
...
if(!function_exists('mbw_get_template_index')){
	function mbw_get_template_index(){
		global $mstore;
		return $mstore->get_template_index();
	}	
}
if(!function_exists('mbw_set_template_index')){
	function mbw_set_template_index($value){
		global $mstore;
		return $mstore->set_template_index($value);
	}	
}
