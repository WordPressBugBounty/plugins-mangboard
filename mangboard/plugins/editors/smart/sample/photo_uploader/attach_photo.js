    ...
    function addImage(ofile){
		...
		if( !!bExceedLimitTotalSize ){
	        ...
	        aFileList.push('	<a onclick="delImage(\'img'+nImageInfoCnt+'\')"><img class="del_button" src="'+mb_editor_url+'sample/photo_uploader/img/btn_del.png"  width="14" height="13" alt="첨부 사진 삭제"></a>');
			aFileList.push('	</li> ');   
    ...
    function html5Upload() {	
		...
		if(typeof(opener) != "undefined"){
			...
			if(board_name=="" && opener.parent.mb_options["board_name"]){
				board_name		= opener.parent.mb_options["board_name"];			//mangboard wp edit
			}
		}
		sUploadURL= ajax_url+"?mode=html5&action=mb_uploader&board_name="+board_name+"&"+mb_editor_nonce;		//mangboard wp edit		
 		...
 		try{
	    	...
	    	for (var i = 0; i < aTemp.length ; i++){
	    		if( !!aTemp[i] && aTemp[i] != "" && aTemp[i].indexOf("=") > 0){
					...
					//mangboard wp edit
					if(typeof(aSubTemp[2])==='undefined'){
						htTemp[aSubTemp[0]] = decodeURIComponent(aSubTemp[1]);
					}else{
						htTemp[aSubTemp[0]] = aSubTemp[1]+"="+aSubTemp[2];
					}	    			
	    		}
	 		}
 		}catch(e){}
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
		...
		if(typeof(opener) != "undefined"){
			...
			if(opener.parent.mb_urls["home"]){
				home_url			= opener.parent.mb_urls["home"];			//mangboard wp edit
			}
			if(board_name=="" && opener.parent.mb_options["board_name"]){
				board_name		= opener.parent.mb_options["board_name"];			//mangboard wp edit
			}
