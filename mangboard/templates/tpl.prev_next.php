	...
	function mbw_get_prev_next_html($data){
		...
		if($field_check){
			...
			if(isset($mb_fields["select_board"]["fn_gid"]) && isset($mb_fields["select_board"]["fn_reply"]) && mbw_get_param("order_by")=="fn_pid"){
				...
				if(empty($check_field1)){
					$check_field1		= "fn_gid";
				}else if(strpos($check_field1, "fn_")!==0){
					$check_field1		= "fn_".$check_field1;
				}
				$check_field2		= "fn_pid";
			}
			...
			if(isset($mb_fields["select_board"][$check_field1])){
			...
			*/
			...
			for($i=0;$i<$count;$i++){
				if($cols_width[$i]!="" && $cols_width[$i]!="*")
					$prev_next_html	= $prev_next_html.'<col style="width:'.esc_attr($cols_width[$i]).'"/>';
				else
...
if(!function_exists('mbw_get_prev_next_template')){
	function mbw_get_prev_next_template($mode,$fields,$data,$echo=true){
		...
		for($i=0;$i<$pn_count;$i++){
			...
			$template_start	.= '<td class="pn_'.esc_attr($item_name).'">';
			
			$item_value	= $data[$mb_fields["select_board"][$pn_fields[$i]]];
			if(has_filter('mf_board_sitem')){
				$filter_item	= apply_filters("mf_board_sitem", array("value"=>$item_value,"field"=>$pn_fields[$i],"type"=>"prev_next"), $data);
				$item_value	= $filter_item["value"];
			}			
			if(has_filter('mf_board_item')){
				$filter_item	= apply_filters("mf_board_item", array("value"=>$item_value,"field"=>$pn_fields[$i],"type"=>"prev_next"), $data);
				$item_value	= $filter_item["value"];
			}
